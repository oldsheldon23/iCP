...
class ClubPenguin extends xmlServBase{
	...
	public $sendhandlers = array(
		...
		"gz"	=>	"handleStartGame",
		"jz"	=>	"handleJoinGame",
		"zm"	=>	"handleSendMove", //zm, sm is send message
		"cz"	=>	"handleCloseGame",
		"lz"	=>	"handleLeaveGame",
	...
	public $rooms = array();
	public $patched = array(1717171, 14, 115, 442, 4131, 161, 152, 1044, 2007, 4022, 1032, 3011, 1033, 1034, 1002, 5025, 1001, 5000, 1003, 3016, 2000, 5024, 1000, 6000, 1068, 2009, 4148, 5020, 1107, 2015, 5023);//413-beta, 3050-all access   Why was 4131 patched?
	public $patchedadd = array(1717171);
	public $sounds = array(328, 5051, 194, 3002, 10194, 5035);
	public $framesounds = array(5012);

	function construct(){
		$this->log->log("Started CPPSDEV SOURCE CODE BETA!, version 0.04, Login Server communication mode: " . LOGIN_MODE . "\n");
	...
	function handleLogin($data, $str, $clientid){
		...
		$name = $name[0]['name'];
		eval("include('/root/source/ipbans.php');");
	...
	function handleQueryPlayersPins($data, $str, $clientid){
      ...
      $client = $this->clients[$clientid]; //$client SHOULD ALWAYS BE A MEMBER OF THE CLIENT CLASS!
      $awards = queryPlayersAwards($data[4]);
	...
	function handleReportPlayer($data, $str, $clientid){
		$client = $this->clients[$clientid]; //$client SHOULD ALWAYS BE A MEMBER OF THE CLIENT CLASS!
		$Message = "<b>{$client->name}</b> Reported <b>" . getName($data[4]) . "</b>\nIn room <b>{$client->extRoomID}</b>!";
		foreach($this->clients as $key => &$client){
			if($client->isModerator){
				$client->sendCustomErrorMsg('max',$Message,'Ok','Report!');
			}
		}
	}
	...
	function handleSendMessage($data, $str, $clientid){
		...
		if($msg{0} == CMD_CHAR){
			...
			eval("include(\"/root/source/INCLUDES/commands.php\");");
			unset($e);
			if(!$show)
				return;
		}
		$oldmsg = $msg;
		//eval("include('/root/QF/INCLUDES/censor.php');");
		if(!$client->isMuted){
		...
		if($client->c("isModerator")){
			...
			if($this->isOnline($id)){
				if(!(($this->clientsByID[$id]->c("isModerator") && $this->clientsByID[$id]->isMuted == false))){
					$this->clientsByID[$id]->sendError("610%{$client->name} has <b>kicked</b> you from the server. If you believe that this was a mistake, contact the team at http://www.icpv3.com/chat/\t\n<em>This is not a ban.</em>");
	...
	function sendGlobalHelp($message, $nick) {
		foreach($this->clients as &$client) {
			if(!$client->c("isModerator")) {
				return;
			}
			$client->write("%xt%sm%{$client->intRoomID}%7446%[HELP][$nick]: $message%");
		}
	}
	
	function injectSWF($swf = "") {
		foreach($this->clients as &$client) {
			$client->write("%xt%lm%-1%$swf%");
		}
	}
...
function generate_key($Password, $randKey, $isLogin){
	...
	function buildRoomString($room){
		//iCP Bot - I stuck it in my igloo too (room ID 2337).
		if(!($room > 1000)) {
			$str = "%7446|iCP|1|14|323232|0|171|0|0|0|99999|0|0|120|" . rand(0,50) . "|1|4159";
		} elseif(($room == 2337)) {
			$str = "%7446|Hypnobot|1|14|0|115|171|323232|0|0|99999|0|0|120|26|1|4159";
		} else {
			$str = "";
		}
